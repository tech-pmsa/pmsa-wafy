...
// Custom tooltip for the bar chart for a cleaner look
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-lg border bg-background p-2 shadow-sm">
        <p className="font-bold">{label}</p>
        <p className="text-sm text-primary">{`Attendance: ${payload[0].value.toFixed(1)}%`}</p>
      </div>
    );
  }
  return null;
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">College Attendance Dashboard</h1>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <StatCard
              ...
              icon={Users}
              footer={`Average attendance for ${selectedClass}`}
            />
            <StatCard
              title="Top Performer"
              value={classData.topPerformer?.name || 'N/A'}
              icon={TrendingUp}
              footer={`${classData.topPerformer?.percentage.toFixed(1)}% Attendance`}
            />
            <StatCard
              title="Below 75% Attendance"
              value={classData.belowThreshold.toString()}
              icon={AlertTriangle}
              footer={`Students needing attention`}
            />
          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Toggle between chart and detailed list view.
                </CardDescription>
              </div>
