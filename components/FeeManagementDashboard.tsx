...
// Component for rendering a single sheet's content (for admins/teachers)
function FeeSheetContent({
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      row =>
        row[1]?.toLowerCase().includes(lowercasedFilter) || // CIC
        row[2]?.toLowerCase().includes(lowercasedFilter)    // Name
    );
  }, [rows, searchTerm]);
...
export default function FeeManagementDashboard() {
  const { loading, role, details } = useUserData() // CORRECTED: Using the new hook
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [role, loading, details]); // CORRECTED: Dependency array now uses details
